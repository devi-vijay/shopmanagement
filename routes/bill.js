...
//making  bill and it is storing in generated_pdf folder
router.post('/generateReport', auth.authenticateToken, (req,res)=>{
  ...
  connection.query(query, [orderDetails.name, generatedUuid,orderDetails.email,orderDetails.contactNumber,orderDetails.paymentMethod,orderDetails.totalAmount,orderDetails.productDetais,res.locals.email],(err,results)=>{
      if (!err) {
...
//get single user bills
router.get('/getBills/:id',auth.authenticateToken,(req,res,next)=>{
  const billId = req.params.id;
  const query ="select *from bill where id = ${billId}";
  connection.query(query,(err,results)=>{
    if(!err){
      if(results.length === 0){
